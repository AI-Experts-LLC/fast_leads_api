#!/usr/bin/env python3
"""
Mercy Medical Center (Springfield, MA) Prospect Discovery Test
Tests the improved filtering with detailed funnel report
"""
...
# Mercy Medical Center Configuration
MERCY_DATA = {
    "company_name": "Mercy Medical Center",
    "company_city": "Springfield",
    "company_state": "Massachusetts",
    "salesforce_account_id": "001VR00000UhY4iYAF"
}

def test_mercy_discovery():
    """Test improved prospect discovery for Mercy Medical Center"""

    print("=" * 80)
    print("🏥 MERCY MEDICAL CENTER (SPRINGFIELD, MA) - PROSPECT DISCOVERY TEST")
    print("=" * 80)
    print(f"Company: {MERCY_DATA['company_name']}")
    print(f"Location: {MERCY_DATA['company_city']}, {MERCY_DATA['company_state']}")
    print(f"Salesforce ID: {MERCY_DATA['salesforce_account_id']}")
    print()
    ...
    print("🔎 Starting improved prospect discovery with enhanced filtering...")
    ...
    try:
        response = requests.post(
            ...
            json={
                "company_name": MERCY_DATA["company_name"],
                "company_city": MERCY_DATA["company_city"],
                "company_state": MERCY_DATA["company_state"]
            },
            timeout=600
        )
        ...
        if response.status_code == 200:
            ...
            if data.get('status') == 'success':
                ...
                print(f"\n📋 FILTERED OUT PROSPECTS (showing first 15):")
                print("-" * 80)
                filtered_details = funnel.get('detailed_filtered_prospects', [])
                for i, filtered in enumerate(filtered_details[:15], 1):
                ...
                if len(filtered_details) > 15:
                    print(f"   ... and {len(filtered_details) - 15} more\n")
                ...
                filename = f"mercy_springfield_{timestamp}.json"
